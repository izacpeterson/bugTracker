...
if (process.env.prod == "true") {
  ...
  db.run(
    "CREATE TABLE IF NOT EXISTS todos (name TEXT, uuid TEXT, status TEXT)"
  );
} else {
  console.log("DEV DB");
  db = new sqlite3.Database(":memory:");
  db.run(
    "CREATE TABLE todos (name TEXT, uuid TEXT, status TEXT, project TEXT)",
    (err) => {
      if (err) {
        console.log(err);
      } else {
...
function addTestData() {
  ...
  testValues.forEach((todo) => {
    let query =
      "INSERT INTO todos (name, project, uuid, status) VALUES(?, ?, ?, ?)";
...
router.get("/add", (req, res) => {
  let uuid = uuidv4();
  const query =
    "INSERT INTO todos(name, status, project, uuid) VALUES(?, ?, ?, ?)";
